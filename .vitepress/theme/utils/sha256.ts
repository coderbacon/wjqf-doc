/**
 * 纯 JavaScript SHA-256 实现
 * 用于 HTTP 环境下 crypto.subtle 不可用时的降级方案
 *
 * 基于 FIPS 180-4 标准实现，与 Web Crypto API 输出完全一致
 */
export function sha256(input: string): string {
  // UTF-8 编码
  const msg = unescape(encodeURIComponent(input))
  const msgLen = msg.length

  // 将字节打包为 32-bit 大端字
  const words: number[] = []
  for (let i = 0; i < msgLen; i++) {
    words[i >> 2] |= msg.charCodeAt(i) << (24 - (i % 4) * 8)
  }

  // 填充：先加 0x80，再加 0x00 直到 448 mod 512，最后 64 位存原始长度
  words[msgLen >> 2] |= 0x80 << (24 - (msgLen % 4) * 8)
  const totalBits = msgLen * 8
  const endIndex = (((msgLen + 8) >> 6) + 1) * 16
  for (let j = (msgLen >> 2) + 1; j < endIndex; j++) {
    words[j] = 0
  }
  words[endIndex - 1] = totalBits >>> 0
  words[endIndex - 2] = Math.floor(totalBits / 0x100000000)

  // SHA-256 初始哈希值
  const H = [
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
    0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
  ]

  const K = [
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
    0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
    0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
    0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
  ]

  for (let i = 0; i < words.length; i += 16) {
    const W = words.slice(i, i + 16)
    for (let t = 16; t < 64; t++) {
      const s0 = (rr(W[t - 15], 7) ^ rr(W[t - 15], 18) ^ (W[t - 15] >>> 3)) >>> 0
      const s1 = (rr(W[t - 2], 17) ^ rr(W[t - 2], 19) ^ (W[t - 2] >>> 10)) >>> 0
      W[t] = (W[t - 16] + s0 + W[t - 7] + s1) >>> 0
    }

    let [a, b, c, d, e, f, g, h] = H
    for (let t = 0; t < 64; t++) {
      const S1 = (rr(e, 6) ^ rr(e, 11) ^ rr(e, 25)) >>> 0
      const ch = ((e & f) ^ (~e & g)) >>> 0
      const temp1 = (h + S1 + ch + K[t] + W[t]) >>> 0
      const S0 = (rr(a, 2) ^ rr(a, 13) ^ rr(a, 22)) >>> 0
      const maj = ((a & b) ^ (a & c) ^ (b & c)) >>> 0
      const temp2 = (S0 + maj) >>> 0

      h = g; g = f; f = e; e = (d + temp1) >>> 0
      d = c; c = b; b = a; a = (temp1 + temp2) >>> 0
    }

    H[0] = (H[0] + a) >>> 0
    H[1] = (H[1] + b) >>> 0
    H[2] = (H[2] + c) >>> 0
    H[3] = (H[3] + d) >>> 0
    H[4] = (H[4] + e) >>> 0
    H[5] = (H[5] + f) >>> 0
    H[6] = (H[6] + g) >>> 0
    H[7] = (H[7] + h) >>> 0
  }

  return H.map((v) => v.toString(16).padStart(8, '0')).join('')
}

function rr(x: number, n: number): number {
  return ((x >>> n) | (x << (32 - n))) >>> 0
}
